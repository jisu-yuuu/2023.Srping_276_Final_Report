rm(list = ls())
#rebalancing for high portfolio using maximum expected return
...
tickers = c("JNJ","MRK","ABBV","AMGN","USPH","NRC","CHE","IQV","MTD","IDXX","PFE",
            "HAE","JAZZ","STE","ELV","DXCM","RVNC","PODD")
...
write.csv(rt_daily, file = "rt_daily_high.csv", row.names=F) 
...
port_order = data.frame(var = sqrt(diag(varcov)), mu)
...
weight = maxSharpe(m = as.numeric(mu), var = matrix(varcov, ncol = length(ticker_select)), 
          wmin = -0.2, wmax = 1.2, groups.wmin = -0.2, groups.wmax = 1.2)    
...
for (j in 1: rebal_freq){
  ...
  # Calculate weights with maxsharpe
  ...
  weight = maxSharpe(m = as.numeric(mu), var = matrix(varcov, ncol = length(ticker_select2)), 
                     wmin = -0.2, wmax = 1.2, groups.wmin = -0.2, groups.wmax = 1.2)   
...
write.csv(old_port.wts, file = "portfolio_weights_months_high.csv", row.names=F) 
write.csv(rt_ptf, file = "return_portfolio_months_high.csv", row.names=F)
write.csv(port_val, file = "port_val_months_high.csv", row.names=F)
write.csv(ticker_selected, file = "tickers_high.csv", row.names=F)
write.csv(month_fee, file = "fee_high.csv", row.names=F)
write.csv(rt_daily, file = "rt_daily_high.csv", row.names=F) 
